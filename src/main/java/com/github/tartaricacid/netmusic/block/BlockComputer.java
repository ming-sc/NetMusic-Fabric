...
public class BlockComputer extends HorizontalFacingBlock {
    ...
    private static VoxelShape rotateShape(Direction from, Direction to, VoxelShape shape) {
        ...
        for (int i = 0; i < times; i++) {
            buffer[0].forEachBox((minX, minY, minZ, maxX, maxY, maxZ) -> {
                buffer[1] = VoxelShapes.union(buffer[1], VoxelShapes.cuboid(new Box(1 - maxZ, minY, minX, 1 - minZ, maxY, maxX)));
            });
            buffer[0] = buffer[1];
    ...
    public NamedScreenHandlerFactory createScreenHandlerFactory(BlockState state, World world, BlockPos pos) {
        return new SimpleNamedScreenHandlerFactory((id, inventory, player) -> new ComputerMenu(id, inventory), new LiteralText("computer"));
    }
