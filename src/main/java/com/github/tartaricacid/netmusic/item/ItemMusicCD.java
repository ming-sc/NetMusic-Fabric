...
public class ItemMusicCD extends Item {
    ...
    public static SongInfo getSongInfo(ItemStack stack) {
        if (stack.getItem() == InitItems.MUSIC_CD) {
            ...
            if (tag != null && tag.contains(SONG_INFO_TAG, new CompoundTag().getType())) {
    ...
    public static class SongInfo{
        ...
        public SongInfo(CompoundTag nbt) {
            this.songUrl = nbt.getString("url");
            this.songName = nbt.getString("name");
            this.songTime = nbt.getInt("time");
            if (nbt.contains("trans_name", NbtType.STRING)){
                this.transName = nbt.getString("trans_name");
            }
            if (nbt.contains("vip", NbtType.BYTE)) {
                this.vip = nbt.getBoolean("vip");
            }
            if (nbt.contains("read_only", NbtType.BYTE)) {
                this.readOnly = nbt.getBoolean("read_only");
            }
            if (nbt.contains("artists", NbtType.LIST)) {
                ListTag nbtList = nbt.getList("artists", 8);
                this.artists = Lists.newArrayList();
                nbtList.forEach(tag -> this.artists.add(tag.asString()));
            }
        }

        public static SongInfo deserializeNBT(CompoundTag nbt){
            return new SongInfo(nbt);
        }

        public static void serializeNBT(SongInfo info, CompoundTag nbt) {
            ...
            if (info.artists != null && !info.artists.isEmpty()) {
                ...
                info.artists.forEach(artist -> nbtList.add(StringTag.of(artist)));
