    ...
    public static void sendToNearBy(World world, BlockPos pos, Message<?> message) {
        if (world instanceof ServerWorld){
            ...
            serverWorld.getChunkManager().threadedAnvilChunkStorage.getPlayersWatchingChunk(new ChunkPos(pos), false)
                    ...
                    .forEach(p -> ServerSidePacketRegistryImpl.INSTANCE.sendToPlayer(p, message.getPacketId(), buffer));
        }
    }

    public static void sendToClientPlayer(Message<?> message, ServerPlayerEntity player) {
        ServerSidePacketRegistryImpl.INSTANCE.sendToPlayer(player, message.getPacketId(), message.toBuffer());
    }
}
