...
public class InitItems {
    ...
    public static final ItemGroup NET_MUSIC_TAB = new FabricItemGroupBuilderImpl(new Identifier(NetMusic.MOD_ID, "netmusic_group"))
            .displayName(Text.translatable("itemGroup.netmusic.netmusic_group"))
            .icon(() -> new ItemStack(MUSIC_PLAYER))
            .entries((ItemGroup.DisplayContext displayContext, ItemGroup.Entries entries) -> {
                entries.add(new ItemStack(MUSIC_PLAYER));
                entries.add(new ItemStack(CD_BURNER));
                entries.add(new ItemStack(COMPUTER));
                entries.add(new ItemStack(MUSIC_CD));
