...
public class NetMusicCommand {
    ...
    public static int getSong(CommandContext<ServerCommandSource> context) {
        try {
            ...
            ItemStack musicDisc = ItemMusicCD.setSongInfo(songInfo, InitItems.MUSIC_CD.getStackForRender());
            ServerPlayerEntity serverPlayer = context.getSource().getPlayer();
            boolean canPlaceIn = serverPlayer.inventory.insertStack(musicDisc);
